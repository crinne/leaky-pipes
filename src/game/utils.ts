...
export function scaleCanvas(canvas: HTMLCanvasElement, context: CanvasRenderingContext2D, width: number, height: number) {
  ...
  if (devicePixelRatio !== backingStoreRatio) {
    ...
    const el = document.getElementById('leaky-pipes-game')!.getBoundingClientRect()
    const elemRect = canvas.getBoundingClientRect();
    

    // add margin to get left
    if (width - 1344 > 0) {
      canvas.style.marginLeft = ((Math.abs(elemRect.left) + el.left )* 2) + 'px';
    }
